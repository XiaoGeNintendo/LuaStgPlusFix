...
function _straight:init(imgclass,index,x,y,v,angle,aim,omiga,stay,destroyable,time,_495,accel,accangle,maxv,through)
	...
	time = time or 0
	self.rot = angle
	if aim then self.rot = self.rot + Angle(self,player) end
	self.omiga=omiga
...
function _straight:frame()
	if self.through then
		if self.y > 224 then self.y = self.y - 448 self.through = nil end
		if self.y < -224 then self.y = self.y + 448 self.through = nil end
		if self.x > 192 then self.x = self.x - 384 self.through = nil end
		if self.x < -192 then self.x = self.x + 384 self.through = nil end
	end
...
function _set_a(obj,a,rot,aim)
	...
	if aim then rot = rot + Angle(obj,player) end
