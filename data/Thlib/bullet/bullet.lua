...
function bullet:kill()
	...
	if self._index then New(BulletBreak,self.x,self.y,self._index) end
...
--	self.imgclass.del(self)
	...
	if self._index and BoxCheck(self,-192,192,-224,224) then New(BulletBreak,self.x,self.y,self._index) end
end
...
function straight_495:frame()
	if not self.reflected then
		if self.y > 224 then
			self.vy = -self.vy
			if self.acceleration and self.acceleration.ay then self.acceleration.ay = -self.acceleration.ay end
			self.y = 448-self.y
			self.rot = -self.rot
			self.reflected=true
			return
		...
		if self.x > 192 then
			self.vx=-self.vx
			if self.acceleration and self.acceleration.ax then self.acceleration.ax = -self.acceleration.ax end
			self.x=384-self.x
